...
const SuperEncodeur::encoderStateTransition SuperEncodeur::fullPulseTransitionTable[] = {
...
const SuperEncodeur::encoderStateTransition SuperEncodeur::halfPulseTransitionTable[] = {
...
SuperEncodeur::isrInfo SuperEncodeur::_isrTable[CORE_NUM_INTERRUPT];
#endif

SuperEncodeur::SuperEncodeur(uint8_t aPin, uint8_t bPin, uint8_t _button, int16_t minValue,
		int16_t maxValue, int16_t initalValue, uint8_t type) {
	active = false;
	configure(aPin, bPin, _button, minValue, maxValue, initalValue, type);
}

SuperEncodeur::SuperEncodeur() {
	active = false;
	configured = false;
...
void SuperEncodeur::configure(uint8_t aPin, uint8_t bPin, uint8_t cPin, int16_t minValue,
		int16_t maxValue, int16_t initalValue, uint8_t type) {
...
bool SuperEncodeur::begin() {
	...
	_isrTable[_interruptA].functPtr = &SuperEncodeur::aPinChange;
	...
	_isrTable[_interruptB].functPtr = &SuperEncodeur::bPinChange;
...
bool SuperEncodeur::getState(EncoderState &state) {
...
bool SuperEncodeur::getAndSet(int16_t val, EncoderState &Oldstate, EncoderState &Newstate) {
	bool changed;
	if (val < _minValue) {
		val = _minValue;
...
bool SuperEncodeur::newSettings(int16_t newMin, int16_t newMax, int16_t newCurrent, EncoderState &state) {
	if (newMax <= newMin) {
		return false;
	}
...
bool SuperEncodeur::enabled() const {
	return active;
}

void SuperEncodeur::attachCallback(EncoderCallBack cback, void *uPtr) {
	callBackPtr = cback;
	userPointer = uPtr;
}

bool SuperEncodeur::buttonPressed(){
...
int16_t SuperEncodeur::setValue(int16_t val) {
	if (val < _minValue) {
		val = _minValue;
	} else if (val > _maxValue) {
		val = _maxValue;
...
int16_t SuperEncodeur::operator =(int16_t val) {
	if (val < _minValue) {
		val = _minValue;
	} else if (val > _maxValue) {
		val = _maxValue;
...
int16_t SuperEncodeur::getValue() {
#if defined(__AVR__)
	int16_t val;
	noInterrupts();  // 16-bit access not atomic on 8-bit processor
...
SuperEncodeur::operator int16_t() const {
#if defined(__AVR__)
	int16_t val;
	noInterrupts();  // 16-bit access not atomic on 8-bit processor
	val = liveState.currentValue;
...
int16_t SuperEncodeur::getAndSet(int16_t val) {
...
bool SuperEncodeur::newSettings(int16_t newMin, int16_t newMax, int16_t newCurrent) {
	bool success = false;
#if defined(__AVR__)
	noInterrupts();  // 16-bit access not atomic on 8-bit processor
#endif
...
void ESP_ISR SuperEncodeur::aPinChange() {
...
void ESP_ISR SuperEncodeur::bPinChange() {
...
void ESP_ISR SuperEncodeur::pinChangeHandler(uint8_t index) {
	uint8_t newStateVariable;

	newStateVariable = SuperEncodeur::tablePtr[currentStateVariable][index];
...
void ESP_ISR SuperEncodeur::updateValue(uint8_t updatedStateVariable) {
